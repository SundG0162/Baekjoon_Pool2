...
int max_segTree[400001];

int max_build(int node, int start, int end)
{
	if (start == end)
	{
		max_segTree[node] = sequence[start];
		return max_segTree[node];
	}

	int mid = (start + end) / 2;

	long long leftMin = max_build(node * 2, start, mid);
	long long rightMin = max_build(node * 2 + 1, mid + 1, end);

	return max_segTree[node] = max(leftMin, rightMin);
}

int max_query(int node, int start, int end, int qStart, int qEnd)
{
	...
	int mid = (start + end) / 2;
	int leftChild = max_query(node * 2, start, mid, qStart, qEnd);
	int rightChild = max_query(node * 2 + 1, mid + 1, end, qStart, qEnd);
